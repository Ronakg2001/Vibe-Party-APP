...
function getSelectedLinkedEventPost() {
  ...
  return (
    hostedEvents.find((eventPost) => eventPost.id === selectedId) ||
    (typeof getPostById === "function" ? getPostById(selectedId) : null)
  );
}
...
function renderPostEventLinkOptions() {
  ...
  defaultOption.textContent =
    hostedEvents.length > 0 ? "Just a normal post" : "No hosted events yet";
  ...
  hostedEvents.forEach((eventPost) => {
    ...
    const status =
      typeof isEventEndedFromDetails === "function" &&
      isEventEndedFromDetails(eventPost.eventDetails)
        ? "Ended"
        : "Running";
  ...
  if (
    currentValue &&
    hostedEvents.some((eventPost) => eventPost.id === currentValue)
  ) {
...
function updatePostLinkHelper() {
  ...
  if (!linkedEvent) {
    if (hostedEvents.length === 0) {
      helper.textContent =
        "Publish an event first if you want this post to show up as a highlight.";
      helper.className = "text-xs text-amber-300";
      isPublishingCreatePost = false;
      return;
    }
    helper.textContent =
      "Choose one of your hosted events to turn this into a pre-event or post-event highlight.";
  ...
  const isEnded =
    typeof isEventEndedFromDetails === "function" &&
    isEventEndedFromDetails(linkedEvent.eventDetails);
...
function buildLocalPostMediaPayload(files) {
  ...
  normalizedFiles.forEach((file) => {
    ...
    const type = (file.type || "").toLowerCase().startsWith("video/")
      ? "video"
      : "image";
    mediaUrls.push(url);
...
function calculateDurationMinutesForSubmission(
  startDate,
  startTime,
  endDate,
  endTime,
...
function resetPostCreation() {
  ...
  if (state.pendingEventMediaPreviewUrls) {
    state.pendingEventMediaPreviewUrls.forEach((url) =>
...
function confirmAbortPostCreation() {
  return window.confirm(
    "Discard this post? Your caption, media, and event link will be lost.",
  );
}
...
async function handleEventMediaInputChange(event) {
  ...
  const { accepted, longVideoNames, invalidTypeNames } =
    await validateIncomingMedia(files);
  const uniqueAccepted = accepted.filter(
    (file) => !isDuplicatePendingMedia(file),
  );
  ...
  if (longVideoNames.length > 0) {
    messages.push(
      `Skipped ${longVideoNames.length} video(s) longer than 90 seconds.`,
    );
  }
  if (invalidTypeNames.length > 0) {
    messages.push(
      `Skipped ${invalidTypeNames.length} invalid file(s). Only image/video allowed.`,
    );
  }
  if (overflowCount > 0) {
    messages.push(
      `Only 10 media files are allowed. ${overflowCount} file(s) were not added.`,
    );
  }
...
async function handlePostSubmit(e) {
  ...
  const captionValue = state.isEventMode
    ? (eventBioValue ?? "")
  ...
  if (state.isEventMode) {
    ...
    if (!finalPoint) {
      setLocationStatus(
        "Please detect location or pick location from map.",
        true,
      );
      isPublishingCreatePost = false;
      return;
    ...
    const ticketType =
      document.querySelector('input[name="ticket_type"]:checked')?.value ||
      "Free";
    if (ticketType === "Paid") {
      if (!ticketTiers || ticketTiers.length === 0) {
        setLocationStatus(
          "Please add at least one ticket tier for a paid event.",
          true,
        );
        isPublishingCreatePost = false;
        return;
      ...
      for (const tier of ticketTiers) {
        if (!tier.name || tier.price === "") {
          setLocationStatus(
            "Please fill out all ticket tier names and prices.",
            true,
          );
          isPublishingCreatePost = false;
          return;
    ...
    const customEventType = (
      document.getElementById("event-type-other")?.value || ""
    ).trim();
    ...
    const effectiveEndTime =
      selectedEndTime || (selectedEndDate ? "23:59" : "");
    ...
    if (
      typeof isFutureEventDateTime === "function" &&
      !isFutureEventDateTime(selectedDate, effectiveStartTime)
    ) {
    ...
    if (
      (selectedEndDate || effectiveEndTime) &&
      durationDisplayEl?.value?.trim() &&
      !durationDisplayEl.readOnly
    ) {
    ...
    if (effectiveEndTime) {
      ...
      if (
        typeof isFutureEventDateTime === "function" &&
        !isFutureEventDateTime(effectiveEndDate, effectiveEndTime)
      ) {
      ...
      if (typeof calculateDurationMinutesFromTimes === "function") {
        const computedDuration = calculateDurationMinutesForSubmission(
          selectedDate,
          effectiveStartTime,
          selectedEndDate,
          effectiveEndTime,
      ...
      if (
        durationDisplayEl?.value?.trim() &&
        typeof parseDurationDisplayToMinutes === "function"
      ) {
        const parsedManualDuration = parseDurationDisplayToMinutes(
          durationDisplayEl.value,
        );
        if (parsedManualDuration <= 0) {
          setLocationStatus(
            "Invalid duration format. Use 90, 1h 30m, 2h, or 45m.",
            true,
          );
          isPublishingCreatePost = false;
          return;
        ...
        if (durationInputEl)
          durationInputEl.value = String(parsedManualDuration);
        if (typeof formatDurationMinutes === "function") {
    ...
    const startLabel = [selectedDate, effectiveStartTime]
      .filter(Boolean)
      .join(" ");
    ...
    try {
      ...
      if (ticketType === "Paid") {
        ...
        const minTierPrice = Math.min(
          ...ticketTiers.map((tier) => Number(tier.price) || 0),
        );
      ...
      if (typeof postFormData === "function") {
        ...
        if (result?.event) {
          ...
          if (typeof serverEventToPost === "function") {
            ...
            if (Array.isArray(state.hostedEventPosts)) {
              state.hostedEventPosts = [
                createdEventPost,
                ...state.hostedEventPosts.filter(
                  (item) => item.id !== createdEventPost.id,
                ),
              ];
            }
          ...
          if (typeof renderHostedEventList === "function")
      ...
      if (activityId && typeof finishUploadActivity === "function")
        finishUploadActivity(activityId, false);
  ...
  if (!trimmedCaption && state.pendingEventMedia.length === 0) {
    if (typeof setLocationStatus === "function") {
      setLocationStatus(
        "Add a caption or at least one photo/video before publishing.",
        true,
      );
    }
    isPublishingCreatePost = false;
  ...
  if (linkedEvent?.eventDetails) {
    ...
    newPost.linkedEventTitle =
      linkedEvent.eventDetails.title || "Untitled Event";
  }
...
function initPostSwipePanel() {
  ...
  if (!panel || !thumb || !bg || !icon || !left || !right || postSwipeReady)
    return;
  postSwipeReady = true;
...
function resetEventCreation() {
  ...
  document
    .querySelectorAll("#services-container .service-checkbox")
    .forEach((input) => {
      input.checked = false;
    });
  document
    .querySelectorAll('#services-container [data-custom-service="true"]')
  ...
  const freeTicket = document.querySelector(
    'input[name="ticket_type"][value="Free"]',
  );
...
function validateCurrentStep() {
  ...
  if (currentStep === 1) {
    ...
    if (
      eventType === "Other" &&
      !document.getElementById("event-type-other")?.value?.trim()
    ) {
...
function getRemainingSeats() {
  ...
  const totalCapInput =
    parseInt(document.getElementById("inp-capacity")?.value) || 0;
  ...
  if (field === "qty") {
    ...
    if (isCapacityOn && !isTotalFlex) {
      ...
      if (requested > maxAllowed) {
        alert(
          `Limit reached! You only have ${maxAllowed} seats available out of your total limit.`,
        );
...
function renderTiers() {
  ...
  if (ticketTiers.length === 0) {
    container.innerHTML =
      '<p class="text-xs text-gray-400 italic">No sections added yet.</p>';
  } else {
    container.innerHTML = ticketTiers
      .map(
        (tier, index) => `
...
function generatePreview() {
  const title =
    document.getElementById("event-title")?.value || "Untitled Event";
  const category =
  ...
  const bio =
    document.getElementById("event-bio")?.value ||
    document.getElementById("post-caption")?.value ||
    "No description provided.";
  const venue =
    document.getElementById("event-location")?.value || "Location TBD";
  ...
  document.getElementById("prev-venue").innerHTML =
    `<i data-lucide="map-pin" class="w-3.5 h-3.5 inline"></i> ${venue}`;
  ...
  const tagsHtml = eventTags
    .map(
      (tag) =>
        `<span class="bg-fuchsia-500/20 text-fuchsia-400 border border-fuchsia-500/30 px-2.5 py-1 rounded text-xs font-medium">#${tag}</span>`,
    )
    .join("");
  const servicesHtml = selectedServices
    .map(
      (s) =>
        `<span class="bg-slate-800 text-gray-300 border border-white/10 px-2.5 py-1 rounded text-xs font-medium"><i data-lucide="check" class="w-3 h-3 inline mr-1"></i>${s}</span>`,
    )
    .join("");
  document.getElementById("prev-tags-services").innerHTML =
    tagsHtml + servicesHtml;
  ...
  if (capacityOn && capacityTotal) {
    ...
    document.getElementById("prev-capacity").textContent =
      capacityTotal + (isTotalFlex ? " (Flex)" : " Seats");
  } else {
  ...
  if (ticketType === "Paid" && ticketTiers.length > 0) {
    ...
    document.getElementById("prev-tiers-list").innerHTML = ticketTiers
      .map(
        (t) => `
