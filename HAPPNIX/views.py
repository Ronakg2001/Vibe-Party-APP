...
def create_event_api(request):
    ...
    profile = getattr(request.user, "profile", None)
    if not profile or not profile.gov_id_verified:
        return _error("Action restricted: Party host karne ke liye aapka Aadhaar / Government ID verify hona zaroori hai.", status=403)
