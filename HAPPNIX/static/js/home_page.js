...
function openBookingModal(postId) {
    ...
    bindBookingActionButton(postId); modal.classList.remove('hidden'); modal.scrollTop = 0; if (content) content.scrollTop = 0; setTimeout(() => { modal.classList.remove('opacity-0'); if (window.innerWidth >= 768) content.classList.remove('translate-y-10'); else content.classList.remove('translate-y-full'); }, 10); renderEventQrCode(postId);
}
async function confirmBooking() {
...
function eventQrMarkup(postId) { return `<div class="rounded-2xl border border-white/10 bg-white/[0.03] p-4"><div class="flex flex-col items-center gap-3 text-center"><div><div class="text-[11px] uppercase tracking-[0.18em] text-gray-500">Event QR</div><div class="mt-1 text-sm text-gray-300">Scan to reopen this event</div></div><div class="rounded-2xl bg-white p-3 shadow-lg"><div id="event-qr-container" class="h-36 w-36"></div></div></div></div>`; }
function ticketQrMarkup(ticket, participants) { return `<div class="rounded-3xl border border-white/10 bg-white p-4 text-slate-950"><div class="flex flex-col items-center gap-4 md:flex-row md:items-start md:justify-between"><div><div class="text-[10px] font-black uppercase tracking-[0.24em] text-slate-500">Entry QR</div><div class="mt-2 text-sm font-bold">${escapeHtml(ticket.event.eventDetails.title || 'Event Ticket')}</div><div class="mt-1 text-xs text-slate-500">Ticket ${escapeHtml(String(ticket.id || ''))}</div><div class="mt-1 text-xs text-slate-500">Tier ${escapeHtml(ticket.tierName || 'General')}</div></div><div class="rounded-2xl bg-slate-100 p-3 shadow-sm"><div id="ticket-qr-modal-container" class="h-36 w-36"></div></div></div><div class="mt-4 grid grid-cols-2 gap-2 text-[11px]"><div><span class="font-bold">Booked by:</span> @${escapeHtml(ticket.bookedByUsername || ticket.username)}</div><div><span class="font-bold">Group:</span> ${escapeHtml(ticket.groupCode || ticket.id)}</div><div><span class="font-bold">Date:</span> ${escapeHtml(ticket.event.eventDetails.date || 'Date TBD')}</div><div><span class="font-bold">People:</span> ${participants.length}</div></div></div>`; }
function buildQrCode(containerId, data, options = {}) { const qrContainer = document.getElementById(containerId); if (!qrContainer || typeof QRCodeStyling === 'undefined') return; qrContainer.innerHTML = ''; const startColor = options.startColor || '#22d3ee'; const endColor = options.endColor || '#d946ef'; const qrCode = new QRCodeStyling({ width: options.size || 144, height: options.size || 144, data, margin: 4, qrOptions: { typeNumber: 0, mode: 'Byte', errorCorrectionLevel: 'H' }, image: '/static/img/HX_Logo.PNG', imageOptions: { crossOrigin: 'anonymous', margin: 3, imageSize: 0.36, hideBackgroundDots: true }, dotsOptions: { type: 'dots', gradient: { type: 'linear', rotation: 0.785398, colorStops: [{ offset: 0, color: startColor }, { offset: 1, color: endColor }] } }, backgroundOptions: { color: 'transparent' }, cornersSquareOptions: { color: '#0f172a', type: 'extra-rounded' }, cornersDotOptions: { color: endColor, type: 'dot' } }); qrCode.append(qrContainer); }
function renderEventQrCode(postId) { buildQrCode('event-qr-container', `${window.location.origin}/home/?event=${encodeURIComponent(String(postId || ''))}`, { size: 144 }); }
function renderTicketQrCode(ticket) { const isVipTier = String(ticket.tierName || '').trim().toLowerCase().includes('vip'); buildQrCode('ticket-qr-modal-container', `${window.location.origin}/ticket/${ticket.id}/`, { size: 144, startColor: isVipTier ? '#f59e0b' : '#22d3ee', endColor: isVipTier ? '#facc15' : '#d946ef' }); }
...
function openTicketDetailsModal(ticketId) {
    ...
    const searchInput = document.getElementById('ticket-detail-user-search'); if (searchInput) searchInput.addEventListener('input', () => searchGroupInvitees(ticket.id, searchInput.value)); if (window.lucide && typeof window.lucide.createIcons === 'function') window.lucide.createIcons(); renderTicketQrCode(ticket);
}
