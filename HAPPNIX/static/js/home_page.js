...
function getStoredDiscoverHistory() {
  try {
    ...
    return Array.isArray(parsed)
      ? parsed.filter((item) => String(item || "").trim())
      : [];
  } catch (_error) {
    return [];
  }
...
function removeDiscoverHistoryItem(query) {
  const trimmed = String(query || "")
    ...
    .toLowerCase();
  syncDiscoverHistory(
    state.discoverHistory.filter((item) => item.toLowerCase() !== trimmed),
  );
}
  ...
  const mediaUrls =
    Array.isArray(post.mediaUrls) && post.mediaUrls.length > 0
      ...
      : post.mediaUrl
        ...
        : [];
  const mediaTypes =
    Array.isArray(post.mediaTypes) &&
    post.mediaTypes.length === mediaUrls.length
      ? post.mediaTypes
      : mediaUrls.map((url) =>
          /\.(mp4|mov|avi|webm|mkv|m4v)$/i.test(url || "") ? "video" : "image",
        );
  return {
    id: String(
...
function persistSharedPost(post) {
  ...
  const normalizedCaption = String(normalized.caption || "")
    .trim()
    .toLowerCase();
  ...
  const nextPosts = [
    ...
    ...getStoredSharedPosts()
      ...
      .filter((item) => {
        ...
        const sameUser =
          Number(item.userId || 0) === Number(normalized.userId || 0);
        const sameCaption =
          String(item.caption || "")
            .trim()
            .toLowerCase() == normalizedCaption;
        const sameMedia =
          JSON.stringify(item.mediaUrls || []) === normalizedMedia;
        ...
        if (
          ...
          sameCaption &&
          sameMedia &&
          createdGap <= recentMatchWindowMs
        ) {
          return false;
        }
        return true;
...
function normalizeStoredTicket(ticket) {
  ...
  return {
    ...
    username: String(
      ticket.username || state.currentUser.username || "",
    ).trim(),
    ...
    event: {
      ...
      eventDetails: {
        ...
        ...eventDetails,
        title: String(
          (normalizedEvent.eventDetails || {}).title ||
            eventDetails.title ||
            event.title ||
            "Untitled event",
        ).trim(),
        date: String(
          (normalizedEvent.eventDetails || {}).date ||
            eventDetails.date ||
            eventDetails.startLabel ||
            "Date TBD",
        ).trim(),
        startLabel: String(
          (normalizedEvent.eventDetails || {}).startLabel ||
            eventDetails.startLabel ||
            eventDetails.date ||
            "",
        ).trim(),
        endLabel: String(
          (normalizedEvent.eventDetails || {}).endLabel ||
            eventDetails.endLabel ||
            "",
        ).trim(),
      },
    },
...
function getActiveTicketForEvent(eventId) {
  ...
  return (
    (state.tickets || []).find((ticket) => {
      ...
      const ticketUsername = String(ticket.username || "")
        .trim()
        .toLowerCase();
      if (currentUserId > 0 && ticketUserId > 0)
...
function isTicketExpired(ticket) {
  return (
    ...
    (ticket?.status === "active" &&
      isEventEndedFromDetails(ticket?.event?.eventDetails))
  );
}
...
async function archiveTicket(ticketId) {
  ...
  const ticket = (state.tickets || []).find(
    (entry) => String(entry.id) === safeTicketId,
  );
  if (!ticket) return;
  try {
...
async function cancelTicket(ticketId) {
  ...
  const applyLocalCancel = () => {
    ...
    if (
      ...
      String(ticket.event?.id || "") === String(currentBookingEventId)
    ) {
  ...
  try {
    ...
    state.tickets = state.tickets.map((entry) =>
      String(entry.id) === safeTicketId ? nextTicket : entry,
    );
    persistTickets();
    renderTicketList();
    if (
...
function isSocialFeedPost(post) {
  ...
  if (
    post.username &&
    String(post.username) === String(state.currentUser.username || "")
  )
    return true;
  return false;
}
...
function hydrateProfileFeedPosts(profile, eventPosts) {
  ...
  const userPosts = (state.posts || [])
    .filter(
      (post) =>
        (safeUserId > 0 && Number(post.userId || 0) === safeUserId) ||
        (safeUsername && post.username === safeUsername),
    )
    .map((post) => ({ ...post, avatar: post.avatar || avatar }));
  return [...(Array.isArray(eventPosts) ? eventPosts : []), ...userPosts].sort(
    (left, right) =>
      new Date(right.createdAt || 0).getTime() -
      new Date(left.createdAt || 0).getTime(),
  );
    ...
    state.followingEventPosts = profileResponses.flatMap((profile) => {
      ...
      const eventPosts = Array.isArray(profile.hosted_events)
        ? profile.hosted_events.map((event) =>
            serverEventToPost({ ...event, hostAvatarUrl: avatar }),
          )
        : [];
      state.publicProfileEventPosts[profile.sql_user_id] =
        hydrateProfileFeedPosts(profile, eventPosts);
      return eventPosts;
    });
  } catch (_error) {
...
function renderUploadActivity() {
  ...
  container.innerHTML = state.uploadActivityItems
    .map(
      (item) => `
...
function finishUploadActivity(id, success = true) {
  ...
  setTimeout(
    () => {
      state.uploadActivityItems = state.uploadActivityItems.filter(
        (entry) => entry.id !== id,
      );
      renderUploadActivity();
    },
...
function getEventStartDate(eventDetails) {
  return (
    parseEventDate(eventDetails?.startLabel) ||
    parseEventDate(eventDetails?.date) ||
    parseEventDate(eventDetails?.startAt)
  );
}

function getEventEndDate(eventDetails) {
  return (
    parseEventDate(eventDetails?.endLabel) ||
    parseEventDate(eventDetails?.endAt)
  );
}
...
function getLiveNowEvents() {
  const seen = new Set();
  const primary =
    Array.isArray(state.liveNowPosts) && state.liveNowPosts.length > 0
      ? state.liveNowPosts
      : [
...
function updateNotificationBadges() {
  const dots = [
    ...
    document.getElementById("desktop-notification-dot"),
  ];
  ...
  listEl.innerHTML = items
    .map((item) => {
      const actorName = escapeHtml(
        item.actor_full_name || item.actor_username || "Someone",
      );
      const actorHandle = escapeHtml(
...
function renderSettingsPrivacyState() {
  ...
  toggleEl.textContent = state.settings.savingPrivacy
    ? "Saving"
    : isPrivate
      ? "Private"
      : "Public";
...
function renderSettingsFollowRequests() {
  ...
  const requests = Array.isArray(state.settings.followRequests)
  ...
  if (state.settings.loading) {
    listEl.innerHTML =
      '<div class="py-10 text-center text-sm text-gray-400">Loading follow requests...</div>';
    return;
  }
  if (requests.length === 0) {
    listEl.innerHTML =
      '<div class="rounded-2xl border border-dashed border-white/10 bg-white/[0.02] px-5 py-6 text-sm text-gray-500">No pending follow requests.</div>';
    return;
  }
  listEl.innerHTML = requests
    .map((user) => {
      const isBusy =
        Number(state.settings.processingRequestId || 0) ===
        Number(user.sql_user_id || 0);
      return `
...
function renderNotificationFollowRequests() {
  const countEl = document.getElementById(
    "notifications-follow-requests-count",
  );
...
async function updatePrivateAccountSetting(nextValue) {
  ...
  try {
    const data = await postJson("/api/profile/privacy", {
      isPrivate: !!nextValue,
    });
...
async function handleFollowRequestAction(requesterUserId, action) {
  ...
  renderNotificationFollowRequests();
  try {
    const response = await postJson("/api/profile/follow-requests", {
      requesterUserId: safeUserId,
      action,
    });
    state.settings.followRequests = state.settings.followRequests.filter(
      (user) => Number(user.sql_user_id) !== safeUserId,
    );
    ...
    if (followersEl && response?.followersCount !== undefined)
      followersEl.textContent = String(response.followersCount || 0);
    renderSettingsFollowRequests();
    renderNotificationFollowRequests();
...
function toggleLike(actionEl) {
  ...
  if (
    post.userId &&
    Number(post.userId) !== Number(state.currentUser.id || 0)
  ) {
    logActivity({
      activityType: "like",
...
async function loadCurrentUserProfile() {
  try {
    ...
    if (followersEl)
      followersEl.textContent = String(profile.followers_count || 0);
    if (followingEl)
      followingEl.textContent = String(profile.following_count || 0);
    if (privacyStatusEl) {
...
function getAllPosts() {
  ...
  const fallbackFeed =
    socialFeed.length > 0
      ? socialFeed
      : [...(state.hostedEventPosts || []), ...(state.nearbyEventPosts || [])];
  const seen = new Set();
  return fallbackFeed
    .filter((post) => {
...
function getLinkedEventMeta(post) {
  ...
  const phase = isEventEndedFromDetails(linkedEvent.eventDetails)
    ? "after"
    : "before";
  return {
    eventId: linkedEvent.id,
    title:
      linkedEvent.eventDetails.title ||
      post.linkedEventTitle ||
      "Untitled Event",
    phase,
...
function getEventHighlightPosts(eventPostId) {
  const linkedPosts = state.posts.filter(
    (post) => !post.isEvent && post.linkedEventId === eventPostId,
  );
  return {
    before: linkedPosts.filter(
      (post) => getLinkedEventMeta(post)?.phase === "before",
    ),
    after: linkedPosts.filter(
      (post) => getLinkedEventMeta(post)?.phase === "after",
    ),
  };
}
...
function renderEventHighlightsSection(title, posts, tone) {
  ...
  const eyebrow = isAfter
    ? "Unlocked after the event wraps."
    : "Live while the event is still running.";
...
function renderLocationModalSuggestions(results) {
  ...
  el.innerHTML = results
    .map((item, idx) => {
      const label =
        getLocalityLabelFromAddress(item.address || {}) || item.display_name;
...
function initLocationModalMap() {
  ...
  locationModalMarker = L.marker(defaultCenter, { draggable: true }).addTo(
    locationModalMap,
  );
  ...
  locationModalMap.on("click", async (event) => {
    ...
    updateLocationModalFromMap();
    const locality = await locationModalReverseGeocode(
      event.latlng.lat,
      event.latlng.lng,
    );
    if (locality) {
...
function openLocationModal(context = "default") {
  ...
  if (input) {
    if (context === "event") {
      ...
      input.value =
        state.customLocationName || state.detectedLocationName || "";
    }
  }
...
function updateLocationModalMeta() {
  const detectedLabel = document.getElementById(
    "location-modal-detected-inline",
  );
  if (detectedLabel) {
...
function serverEventToPost(eventData) {
  ...
  const mediaTypeForUrl = (url) =>
    /\.(mp4|mov|avi|webm|mkv|m4v)$/i.test(url || "") ? "video" : "image";
  const primaryIsVideo = mediaTypeForUrl(primaryUrl) === "video";
  const fallbackPrice = Number(eventData.price) || 0;
  const ticketType =
    eventData.ticketType || (fallbackPrice > 0 ? "Paid" : "Free");
  let ticketTiers = Array.isArray(eventData.ticketTiers)
  ...
  return {
    ...
    image:
      eventData.imageUrl ||
      primaryUrl ||
      `https://images.unsplash.com/photo-1545128485-c400e7702796?w=600&h=600&fit=crop&q=${Math.random()}`,
    mediaType: primaryIsVideo ? "video" : "image",
    mediaUrl: primaryUrl || "",
    mediaUrls:
      mediaUrls.length > 0 ? mediaUrls : primaryUrl ? [primaryUrl] : [],
    mediaTypes:
      mediaUrls.length > 0
        ? mediaUrls.map(mediaTypeForUrl)
        : primaryUrl
          ? [mediaTypeForUrl(primaryUrl)]
          : [],
    caption: eventData.description || "Live event nearby.",
    likes: 0,
    isEvent: true,
    createdAt:
      eventData.createdAt || eventData.updatedAt || new Date().toISOString(),
...
async function loadHostedEvents() {
  try {
    ...
    state.hostedEventPosts = (data.events || []).map((event) =>
      serverEventToPost({
        ...event,
    ...
    state.hostedEventPosts = [];
    setLocationStatus(
      error.message || "Failed to load your hosted events.",
      true,
    );
  }
  renderFeed();
  ...
  renderHostedEventList();
  renderProfileGrid();
  if (typeof renderPostEventLinkOptions === "function")
...
async function loadNearbyEvents() {
  if (!state.locationEnabled) {
    ...
    loadLiveNowEvents();
    setLocationStatus(
      "Location is OFF. Turn it ON to auto-detect nearby events.",
    );
    return;
  }
  if (!state.userLocation) {
  ...
  try {
    const data = await getJson(
      `/api/events/nearby?latitude=${state.userLocation.latitude}&longitude=${state.userLocation.longitude}&north=${bbox.north}&south=${bbox.south}&east=${bbox.east}&west=${bbox.west}&radiusKm=${radiusKm}`,
    );
    state.nearbyEventPosts = (data.events || []).map(serverEventToPost);
    setLocationStatus(
      `Showing ${data.count || 0} nearby event(s) within ${radiusKm} km.`,
    );
    renderFeed();
    loadLiveNowEvents();
    renderTopLocationUi();
...
async function detectUserLocationAndLoad() {
  ...
  if (!window.isSecureContext && !isLocalHost) {
    setLocationStatus(
      "Location needs HTTPS or localhost. Open app on http://localhost:8000 (same PC) or use an HTTPS tunnel.",
      true,
    );
    return;
  }
  ...
  navigator.geolocation.getCurrentPosition(
    async (position) => {
      ...
      if (state.detectedLocationName) {
        localStorage.setItem(
          "happnix_detected_location_name",
          state.detectedLocationName,
        );
      }
      setLocationStatus("");
...
function renderEventCategoriesFromManifest() {
  ...
  const categories = Array.isArray(manifestDataTemplate.event_category)
    ...
    : [];
  const finalCategoriesBase =
    categories.length > 0 ? categories : ["Local event"];
  ...
  const selectedValue =
    currentValue && finalCategories.includes(currentValue) ? currentValue : "";
...
function legacy_isAnalogCandidateAllowed(hour12, minute, period) {
  ...
  if (
    isTodayEventDateSelected() &&
    !isFutureEventDateTime(getTodayDateKey(), candidateTime)
  ) {
    return false;
  }
  ...
  if (analogClockState.targetInputId === "event-end-time-display") {
    ...
    if (startTime) {
      const duration = calculateDurationMinutesFromTimes(
        startTime,
        candidateTime,
      );
...
function legacy_getAnalogTargetHiddenInputId() {
  return analogClockState.targetInputId === "event-end-time-display"
    ? "event-end-time"
    : "event-time";
}
...
function legacy_syncEventTimeInputs() {
  ...
  const displayTime = document.getElementById(
    analogClockState.targetInputId || "event-time-display",
  );
  if (!displayTime) return;
  const time24 = to24HourTime(
    analogClockState.hour,
    analogClockState.minute,
    analogClockState.period,
  );
  if (hiddenTime) hiddenTime.value = time24;
  displayTime.value = formatTime12(
    analogClockState.hour,
    analogClockState.minute,
    analogClockState.period,
  );
...
function legacy_updateDurationClearButtonVisibility() {
  ...
  const show =
    ...
    !durationDisplay.readOnly &&
    !!durationDisplay.value.trim();
  clearBtn.classList.toggle("hidden", !show);
}

function legacy_clearDurationSelection() {
  const isConfirmed = window.confirm(
    "Are you sure you want to remove the duration?",
  );
  if (!isConfirmed) {
    return;
  }
...
function legacy_syncEventTimeDependencyUi() {
  ...
  const manualDurationMinutes = parseDurationDisplayToMinutes(
    durationDisplay.value,
  );
...
function legacy_getAnalogDialIndexFromPoint(clientX, clientY) {
  ...
  const angleDeg =
    ((Math.atan2(clientY - cy, clientX - cx) * 180) / Math.PI + 90 + 360) % 360;
...
function legacy_initAnalogClockInteractions() {
  ...
  dial.addEventListener("pointerdown", (event) => {
    ...
    setAnalogValueFromDialIndex(
      getAnalogDialIndexFromPoint(event.clientX, event.clientY),
    );
    renderAnalogClockDial();
    event.preventDefault();
  });
...
function legacy_renderAnalogClockDial() {
  ...
  if (!dial || !hand || !digitalHour || !digitalMinute || !digitalPeriod)
    return;
  ...
  if (secondHand) {
    secondHand.classList.toggle(
      "is-visible",
      analogClockState.liveMode && isTodayEventDateSelected(),
    );
  ...
  digitalMinute.classList.toggle(
    "is-active",
    analogClockState.focus === "minute",
  );
  digitalPeriod.classList.toggle(
    "is-active",
    analogClockState.focus === "period",
  );
}
...
function legacy_openAnalogTimeModal(inputId) {
  ...
  if (analogClockState.targetInputId === "event-end-time-display") {
    ...
    const hasManualDuration =
      parseDurationDisplayToMinutes(durationDisplay?.value || "") > 0 &&
      !durationDisplay?.readOnly;
...
function legacy_getSafeSelectedOrTodayDateKey() {
  ...
  return eventCalendarState.selectedDate < todayKey
    ? todayKey
...
function legacy_renderEventMonthYearPicker() {
  ...
  monthGrid.innerHTML = monthShortNames
    .map((label, idx) => {
      const disabled =
        eventCalendarState.viewYear === today.getFullYear() &&
        idx < today.getMonth();
...
function legacy_renderEventCalendar() {
  ...
  const firstDay = new Date(
    eventCalendarState.viewYear,
    eventCalendarState.viewMonth,
    1,
  );
  const startWeekday = firstDay.getDay();
  const daysInMonth = new Date(
    eventCalendarState.viewYear,
    eventCalendarState.viewMonth + 1,
    0,
  ).getDate();
  const todayKey = getTodayDateKey();
  ...
  grid.querySelectorAll("button[data-date]").forEach((buttonEl) => {
    buttonEl.addEventListener("click", () => {
      eventCalendarState.selectedDate =
        buttonEl.getAttribute("data-date") || "";
      renderEventCalendar();
    });
  });
  renderEventMonthYearPicker();
...
function legacy_saveEventDateSelection() {
  ...
  const displayInput = document.getElementById(
    eventCalendarState.targetInputId || "event-date-display",
  );
  ...
  if (
    hiddenTime?.value &&
    !isFutureEventDateTime(dateValue, hiddenTime.value)
  ) {
    hiddenTime.value = "";
    if (displayTime) displayTime.value = "";
    ...
    hiddenEndTime?.value &&
    !isFutureEventDateTime(dateValue, hiddenEndTime.value)
  ) {
    hiddenEndTime.value = "";
    if (displayEndTime) displayEndTime.value = "";
    recalculateEventDurationFromTimes();
...
function legacy_saveAnalogTimeSelection() {
  ...
  if (dateValue && !isFutureEventDateTime(dateValue, time24)) {
    setLocationStatus(
      "Please select a future time for the selected date.",
      true,
    );
    return;
  }
  ...
  try {
    ...
    if (locationInput) {
      ...
      locationInput.addEventListener("change", async () => {
        const results = await locationModalSearch(
    ...
    const discoverProfileModal = document.getElementById(
    ...
    document.addEventListener("click", (event) => {
      if (
        !event.target.closest(".hosted-event-menu") &&
        !event.target.closest('[data-action="hosted-event-card"]')
      ) {
    ...
    if (typeof HappnixQR !== "undefined") {
      const appDownloadUrl = window.location.origin + "/download";
      HappnixQR.generate("app-download-qr", appDownloadUrl, "event");
    }
    ...
    if (!state.locationEnabled) {
      ...
      setLocationStatus(
        "Location auto-detect disabled on insecure origin. Use localhost/HTTPS.",
      );
    }
  } catch (error) {
...
function renderLiveNow() {
  ...
  container.innerHTML = liveEvents
    .map((post) => {
      ...
      const priceLabel =
        Number(post.eventDetails?.price) > 0
          ? formatInr(post.eventDetails.price)
          : "Free";
      const distanceLabel =
        post.eventDetails?.distanceKm !== undefined
          ? `${post.eventDetails.distanceKm} km away`
          : "Live event";
      return `
...
function isDuplicatePendingMedia(file) {
  return state.pendingEventMedia.some(
    (item) =>
      item.name === file.name &&
      item.size === file.size &&
...
function getUserFollowState(userId) {
  ...
  const discoverUser = (state.discoverResults || []).find(
    (user) => Number(user.sql_user_id) === safeUserId,
  );
  if (discoverUser) return discoverUser;
  const graphUser = (state.followGraph.users || []).find(
    (user) => Number(user.sql_user_id) === safeUserId,
  );
  if (graphUser) return graphUser;
  if (
    state.activeDiscoverProfile &&
    Number(state.activeDiscoverProfile.sql_user_id) === safeUserId
  ) {
...
function renderDiscoverProfileMediaGrid(events) {
  ...
  return `
        <div>
            ...
            <div class="grid grid-cols-3 gap-2">${events
              .slice(0, 6)
              .map(
                (post) => `
...
async function openDiscoverProfile(userId) {
  ...
  modal.classList.remove("hidden");
  contentEl.innerHTML =
    '<div class="py-12 text-center text-sm text-gray-400">Loading profile...</div>';
  requestAnimationFrame(() => {
    modal.classList.remove("opacity-0");
    card.classList.remove("scale-95");
  });
  ...
  try {
    ...
    const avatar =
      state.activeDiscoverProfile?.profile_picture_url || defaultAvatar;
    const eventPosts = Array.isArray(state.activeDiscoverProfile?.hosted_events)
      ? state.activeDiscoverProfile.hosted_events.map((event) =>
          serverEventToPost({ ...event, hostAvatarUrl: avatar }),
        )
      : [];
    state.publicProfileEventPosts[safeUserId] = hydrateProfileFeedPosts(
      state.activeDiscoverProfile,
      eventPosts,
    );
...
function renderFollowGraphList() {
  ...
  const graphType =
    state.followGraph.type === "following" ? "following" : "followers";
  titleEl.textContent = graphType === "followers" ? "Fans" : "Following";

  if (state.followGraph.loading) {
    listEl.innerHTML =
      '<div class="py-12 text-center text-sm text-gray-400">Loading accounts...</div>';
    return;
  }

  const users = Array.isArray(state.followGraph.users)
...
function renderExplore() {
  ...
  if (!query) {
    if (state.discoverHistory.length > 0) {
      ...
      resultsEl.innerHTML = `
                <div class="space-y-2">${state.discoverHistory
                  .map(
                    (item) => `
  ...
  const users = Array.isArray(state.discoverResults)
    ? state.discoverResults
    : [];
  statusEl.textContent =
    users.length > 0
...
async function followDiscoverUser(targetUserId) {
  ...
  const nextAction = relation?.follow_request_pending
    ? "cancel_request"
    : relation?.is_following
      ? "unfollow"
      : "follow";
  try {
    const response = await postJson("/api/users/follow", {
    ...
    const statusEl = document.getElementById("discover-search-status");
    if (statusEl)
      statusEl.textContent = error.message || "Follow update failed";
  }
}
...
function renderProfileGrid() {
  ...
  const myPosts = getAllPosts().filter(
    (post) => post.username === state.currentUser.username,
  );
...
function renderTicketList() {
  ...
  state.tickets.forEach((ticket) => {
    ...
    if (!isCancelled && !expired) {
      const verifyUrl =
        window.location.origin + "/api/tickets/" + ticket.id + "/verify";
      const ticketTier = (ticket.tier_name || "").toLowerCase();
      const qrType = ticketTier.includes("vip")
        ? "ticket_vip"
        : "ticket_regular";
...
function openBookingModal(postId) {
  ...
  let ticketTiers = Array.isArray(event.eventDetails.ticketTiers)
...
async function confirmBooking() {
  ...
  if (!event || isEventEndedFromDetails(event.eventDetails)) {
    setLocationStatus(
      "This event has already ended, so booking is closed.",
      true,
    );
    closeBookingModal();
    return;
  ...
  if (hasActiveTicketForEvent(event.id)) {
    setLocationStatus(
      "You already joined this party. Cancel the existing ticket to join again.",
      true,
    );
    closeBookingModal();
    return;
  ...
  try {
    const data = await postJson("/api/tickets/book", {
    ...
    state.tickets = [
      ...
      ...state.tickets.filter(
        (entry) => String(entry.id) !== String(nextTicket.id),
      ),
    ];
    persistTickets();
    renderTicketList();
    if (
      event.userId &&
      Number(event.userId) !== Number(state.currentUser.id || 0)
    ) {
      logActivity({
...
async function deleteHostedEvent(postId) {
  ...
  try {
    ...
    state.nearbyEventPosts = state.nearbyEventPosts.filter(
      (item) => item.id !== postId,
    );
    state.hostedEventPosts = state.hostedEventPosts.filter(
      (item) => item.id !== postId,
    );
    state.posts = state.posts
      .filter((item) => item.id !== postId)
      .map((item) =>
        item.linkedEventId === postId
          ? { ...item, linkedEventId: null, linkedEventTitle: "" }
          : item,
      );
    state.tickets = state.tickets.filter(
      (ticket) => ticket?.event?.id !== postId,
    );
    persistTickets();
    state.hostedMenuEventPostId = null;
    renderFeed();
...
function renderHostedEventList() {
  ...
  emptyState.classList.add("hidden");
  container.innerHTML = hostedEvents
    .map((eventPost) => {
...
function switchMyEventsTab(tabId) {
  ...
  if (state.myEventsTab === "tickets") {
    if (ticketsBtn)
      ticketsBtn.className =
        "flex-1 py-2.5 rounded-lg text-sm font-bold transition-all bg-slate-700 text-white shadow-lg";
    if (eventBtn)
      eventBtn.className =
        "flex-1 py-2.5 rounded-lg text-sm font-bold transition-all text-gray-400 hover:text-white";
    if (ticketsPanel) ticketsPanel.classList.remove("hidden");
    if (eventPanel) eventPanel.classList.add("hidden");
  } else {
    if (eventBtn)
      eventBtn.className =
        "flex-1 py-2.5 rounded-lg text-sm font-bold transition-all bg-fuchsia-600 text-white shadow-lg shadow-fuchsia-900/20";
    if (ticketsBtn)
      ticketsBtn.className =
        "flex-1 py-2.5 rounded-lg text-sm font-bold transition-all text-gray-400 hover:text-white";
...
function bindHomePageActions() {
    ...
    switch (action) {
      ...
      case "handle-follow-request":
        if (actionEl.dataset.requesterId && actionEl.dataset.requestAction) {
          handleFollowRequestAction(
            actionEl.dataset.requesterId,
            actionEl.dataset.requestAction,
          );
        }
        break;
      ...
      case "shift-post-media":
        ...
        shiftPostMedia(
          ...
          Number(actionEl.dataset.direction || 0),
        );
        break;
  ...
  document.addEventListener("pointerdown", (event) => {
    const cancelledTicketCard = event.target.closest(
      '[data-action="ticket-card"]',
    );
    if (cancelledTicketCard?.dataset.ticketId) {
      const ticket = (state.tickets || []).find(
        (entry) =>
          String(entry.id) === String(cancelledTicketCard.dataset.ticketId),
      );
      if (ticket?.status === "cancelled") {
        startCancelledTicketDeletePress(
          event,
          cancelledTicketCard.dataset.ticketId,
        );
      }
    }
  ...
  document.addEventListener("click", (event) => {
    ...
    if (
      ticketCard &&
      !event.target.closest('[data-action="cancel-ticket"]') &&
      !event.target.closest('[data-action="delete-ticket"]') &&
      !event.target.closest('[data-action="archive-ticket"]')
    ) {
...
function switchTab(tabId) {
  ...
  document.querySelectorAll(".nav-btn").forEach((btn) => {
    ...
    if (btn.dataset.target === tabId) {
      ...
      // Icon Glow
      if (icon)
        icon.classList.add("drop-shadow-[0_0_8px_rgba(232,121,249,0.5)]");
      // Desktop Text Color
      if (text) {
      ...
      if (icon)
        icon.classList.remove("drop-shadow-[0_0_8px_rgba(232,121,249,0.5)]");
      if (text) {
...
function normalizeStoredTicket(ticket) {
  ...
  const status =
    ticket.status === "cancelled"
      ...
      : ticket.status === "pending"
        ...
        : "active";
  const participants =
    Array.isArray(ticket.participants) && ticket.participants.length
      ? ticket.participants.map((participant) => ({
          ...
          inviteStatus: String(participant.inviteStatus || "confirmed")
            .trim()
            .toLowerCase(),
          pendingReason: String(participant.pendingReason || "")
            .trim()
            .toLowerCase(),
          ...
          refundTransactionId: String(
            participant.refundTransactionId || "",
          ).trim(),
      ...
      : [
          {
            ...
            bookedById:
              Number(
                ticket.bookedById || ticket.userId || state.currentUser.id || 0,
              ) || null,
            bookedByUsername: String(
              ticket.bookedByUsername ||
                ticket.username ||
                state.currentUser.username ||
                "",
            ).trim(),
            paidById: Number(ticket.paidById || 0) || null,
            paidByUsername: String(ticket.paidByUsername || "").trim(),
            inviteStatus: String(ticket.inviteStatus || "confirmed")
              .trim()
              .toLowerCase(),
            pendingReason: String(ticket.pendingReason || "")
              .trim()
              .toLowerCase(),
            paymentTransactionId: String(
            ...
            ticketPrice: Number(ticket.ticketPrice || 0) || 0,
            serviceFee: Number(ticket.serviceFee || 0) || 0,
            amountDue: Number(ticket.amountDue || 0) || 0,
            isCurrentUser: true,
  ...
  return {
    ...
    groupCode: String(ticket.groupCode || ticket.id || ""),
    userId: Number(ticket.userId || state.currentUser.id || 0) || null,
    username: String(
      ticket.username || state.currentUser.username || "",
    ).trim(),
    bookedById:
      Number(ticket.bookedById || ticket.userId || state.currentUser.id || 0) ||
      null,
    bookedByUsername: String(
      ticket.bookedByUsername ||
        ticket.username ||
    ...
    qty: Math.max(1, Number(ticket.qty || 1)),
    status,
    tierName: String(ticket.tierName || "General").trim(),
    ticketPrice: Number(ticket.ticketPrice || 0) || 0,
    serviceFee: Number(ticket.serviceFee || 0) || 0,
    amountDue: Number(ticket.amountDue || 0) || 0,
    createdAt: ticket.createdAt || new Date().toISOString(),
    ...
    isExpired: Boolean(ticket.isExpired),
    canPay: Boolean(ticket.canPay || status === "pending"),
    participants,
    event: {
      ...normalizedEvent,
      id: String(normalizedEvent.id || event.id || ""),
...
function ticketStatusMeta(ticket) {
  ...
  if (expired)
    return { label: "Expired", className: "text-rose-300", icon: "clock-3" };
  if (ticket?.status === "pending" && ticket?.pendingReason === "tentative")
    return {
      label: "Tentative",
      className: "text-amber-200",
      icon: "hourglass",
...
function ticketParticipantStatusMeta(participant) {
  ...
  if (participant.status === "cancelled")
    return { label: "Cancelled", className: "text-amber-200" };
  if (
    participant.status === "pending" &&
    participant.inviteStatus === "tentative"
  )
    return { label: "Tentative", className: "text-amber-200" };
  if (
    participant.status === "pending" &&
    participant.pendingReason === "payment"
  )
    return { label: "Pending Payment", className: "text-amber-200" };
  if (participant.status === "pending")
    return { label: "Pending", className: "text-amber-200" };
  return { label: "Joined", className: "text-emerald-300" };
}

function selectedBookingTierIndex() {
  ...
  return [
    {
      ...
      username: String(state.currentUser.username || "").trim(),
      isSelf: true,
    },
    ...(bookingDraftState.invitees || []).map((user) => ({
      ...user,
      isSelf: false,
    })),
  ];
}
function getBookingPaidForUserIds() {
  return Array.from(
    document.querySelectorAll('input[name="booking-pay-for"]:checked'),
  )
...
function updateBookingTotal() {
  ...
  if (!totalEl) return;
  const event = getPostById(currentBookingEventId);
  if (inferEventTicketType(event) === "Free") {
...
function renderBookingSearchResults() {
  ...
  if (!resultsEl) return;
  if (!bookingDraftState.searchResults.length) {
    resultsEl.innerHTML = "";
    return;
  }
  resultsEl.innerHTML = bookingDraftState.searchResults
    .map(
      (user) =>
        `<button type="button" data-action="add-booking-invitee" data-user-id="${user.userId}" class="flex w-full items-center justify-between gap-3 rounded-2xl border border-white/10 bg-white/[0.03] px-3 py-3 text-left hover:border-fuchsia-500/30"><div><div class="font-semibold text-white">@${escapeHtml(user.username)}</div><div class="text-xs text-gray-400">${escapeHtml(user.fullName || "Tap to add to this ticket")}</div></div><span class="text-xs font-bold text-fuchsia-300">Add</span></button>`,
    )
    .join("");
}
async function searchBookingInvitees(query) {
  ...
  try {
    const response = await getJson(
      `/api/users/search?q=${encodeURIComponent(safeQuery)}&limit=6`,
    );
    ...
    bookingDraftState.searchResults = (response.users || [])
      .map((user) => ({
        userId: Number(user.sql_user_id || 0),
        username: String(user.username || "").trim(),
        fullName: String(user.full_name || "").trim(),
      }))
      .filter(
        (user) =>
          user.userId > 0 && user.username && !existingIds.has(user.userId),
      );
  } catch (_error) {
    bookingDraftState.searchResults = [];
  }
...
function bindBookingActionButton(postId) {
  const actionButton = document.getElementById("booking-action-btn");
  ...
  actionButton.style.zIndex = "5";
  actionButton.onclick = (eventLike) => {
...
function openBookingModal(postId) {
  ...
  if (!isEnded && !alreadyJoined && !pendingTicket) {
    ...
    if (searchInput)
      searchInput.addEventListener("input", () =>
        searchBookingInvitees(searchInput.value),
      );
    details
  ...
  setTimeout(() => {
    modal.classList.remove("opacity-0");
    if (window.innerWidth >= 768) content.classList.remove("translate-y-10");
    else content.classList.remove("translate-y-full");
  }, 10);
  renderEventQrCode(postId);
}
async function confirmBooking() {
  const event = getPostById(currentBookingEventId);
  if (!event || isEventEndedFromDetails(event.eventDetails)) {
  ...
  const existingTicket = getActiveTicketForEvent(event.id);
  if (existingTicket && existingTicket.status !== "cancelled") {
    closeBookingModal();
    openTicketDetailsModal(existingTicket.id);
    return;
  }
  const fallbackPrice = Number(event.eventDetails.price) || 0;
  const ticketType = inferEventTicketType(event);
  ...
  if (ticketType === "Paid" && ticketTiers.length === 0)
    ticketTiers = [{ name: "General", price: fallbackPrice }];
  const baseFee = ticketType === "Paid" ? 4 : 0;
  const tierIndex = selectedBookingTierIndex();
  const selectedTier = ticketTiers[tierIndex] ||
    ticketTiers[0] || { name: "General", price: fallbackPrice };
  const inviteeStatuses = {};
  Object.entries(bookingDraftState.inviteeStatuses || {}).forEach(
    ([userId, status]) => {
      inviteeStatuses[userId] =
        String(status || "confirmed")
          .trim()
          .toLowerCase() === "tentative"
          ? "tentative"
          : "confirmed";
    },
  );
  try {
    ...
    const nextTicket = upsertStoredTicket(data.ticket);
    if (
      event.userId &&
      Number(event.userId) !== Number(state.currentUser.id || 0)
    )
    ...
    setTimeout(() => {
      switchTab("tickets");
      if (nextTicket) openTicketDetailsModal(nextTicket.id);
    }, 300);
  } catch (error) {
    setLocationStatus(error.message || "Failed to join this party.", true);
  }
}
function describeParticipantSecondary(ticket, participant) {
  if (participant.refundTransactionId)
    return `Refund: ${escapeHtml(participant.refundTransactionId)}`;
  if (participant.paymentTransactionId)
    return `Payment: ${escapeHtml(participant.paymentTransactionId)}`;
  if (participant.paidByUsername)
    return `Paid by @${escapeHtml(participant.paidByUsername)}`;
  if (
    participant.status === "pending" &&
    participant.inviteStatus === "tentative"
  )
    return "Waiting for confirmation";
  if (participant.status === "pending") return "Waiting for payment";
...
async function searchGroupInvitees(ticketId, query) {
  const safeQuery = String(query || "").trim();
  groupTicketDetailState.ticketId = String(ticketId || "");
  if (!safeQuery) {
    groupTicketDetailState.searchResults = [];
    openTicketDetailsModal(ticketId);
    return;
  }
  const ticket = (state.tickets || []).find(
    (entry) => String(entry.id) === String(ticketId),
  );
  if (!ticket) return;
  try {
    const response = await getJson(
    ...
    groupTicketDetailState.searchResults = (response.users || [])
      .map((user) => ({
        userId: Number(user.sql_user_id || 0),
        username: String(user.username || "").trim(),
        fullName: String(user.full_name || "").trim(),
...
function groupTicketTierOptions(ticket) {
  const tiers = Array.isArray(ticket?.event?.eventDetails?.ticketTiers)
    ? ticket.event.eventDetails.ticketTiers
    : [];
  const fallbackPrice = Number(ticket?.ticketPrice || 0) || 0;
  if (!tiers.length && fallbackPrice > 0)
    return [
      {
        name: ticket.tierName || "General",
        price: fallbackPrice,
        fee: Number(ticket.serviceFee || 0) || 0,
      },
    ];
  return tiers.map((tier) => ({
    ...
    fee: Number(ticket?.serviceFee || 0) || 0,
    services: String(tier?.services || "").trim(),
  }));
}
function ensureDetailTierIndex(ticket) {
  const options = groupTicketTierOptions(ticket);
  const currentIndex = Number(groupTicketDetailState.selectedTierIndex || 0);
  if (!options.length) {
    groupTicketDetailState.selectedTierIndex = 0;
    return 0;
  }
  if (currentIndex >= 0 && currentIndex < options.length) return currentIndex;
  const matchIndex = options.findIndex(
    (tier) =>
      String(tier.name || "")
        .trim()
        .toLowerCase() ===
      String(ticket.tierName || "")
        .trim()
        .toLowerCase(),
  );
  groupTicketDetailState.selectedTierIndex = matchIndex >= 0 ? matchIndex : 0;
  return groupTicketDetailState.selectedTierIndex;
}
function selectedDetailTier(ticket) {
  const options = groupTicketTierOptions(ticket);
  const index = ensureDetailTierIndex(ticket);
  return (
    options[index] || {
      name: ticket.tierName || "General",
      price: Number(ticket.ticketPrice || 0) || 0,
      fee: Number(ticket.serviceFee || 0) || 0,
    }
  );
}
...
async function cancelTicket(ticketId) {
  ...
  try {
    ...
    if (
      nextTicket &&
      currentBookingEventId &&
      String(nextTicket.event?.id || "") === String(currentBookingEventId)
    )
      openBookingModal(currentBookingEventId);
    setLocationStatus("Ticket cancelled. You can join this party again now.");
  } catch (error) {
...
function openTicketDetailsModal(ticketId) {
  ...
  const participants = Array.isArray(ticket.participants)
    ? ticket.participants
    : [];
  const pendingParticipants = participants.filter(
    (participant) => participant.status === "pending",
  );
  if (groupTicketDetailState.ticketId !== safeTicketId) {
    groupTicketDetailState.selectedPendingTicketIds = pendingParticipants.map(
      (participant) => String(participant.ticketId || ""),
    );
    groupTicketDetailState.selectedTierIndex = 0;
  } else {
    const visibleIds = new Set(
      pendingParticipants.map((participant) =>
        String(participant.ticketId || ""),
      ),
    );
    groupTicketDetailState.selectedPendingTicketIds = (
      groupTicketDetailState.selectedPendingTicketIds || []
    ).filter((id) => visibleIds.has(String(id)));
  }
  groupTicketDetailState.ticketId = safeTicketId;
  const ticketType = inferEventTicketType(ticket.event);
  const selectedPending = new Set(
    groupTicketDetailState.selectedPendingTicketIds || [],
  );
  const chosenTier = selectedDetailTier(ticket);
  const selectedCount = pendingParticipants.filter((participant) =>
    selectedPending.has(String(participant.ticketId)),
  ).length;
  const selectedTotal = selectedCount
    ? ((Number(chosenTier.price || 0) || 0) +
        (Number(chosenTier.fee || 0) || 0)) *
      selectedCount
    : 0;
  const searchResults =
    groupTicketDetailState.ticketId === safeTicketId
  ...
  const searchInput = document.getElementById("ticket-detail-user-search");
  if (searchInput)
    searchInput.addEventListener("input", () =>
      searchGroupInvitees(ticket.id, searchInput.value),
    );
  if (window.lucide && typeof window.lucide.createIcons === "function")
    window.lucide.createIcons();
...
function closeTicketDetailsModal() {
  ...
  if (!modal || !content) return;
  modal.classList.add("opacity-0");
  if (window.innerWidth >= 768) content.classList.add("translate-y-10");
  else content.classList.add("translate-y-full");
  setTimeout(() => modal.classList.add("hidden"), 300);
}
async function updateGroupTicket(ticketId, payload) {
  const ticket = (state.tickets || []).find(
    (entry) => String(entry.id) === String(ticketId),
  );
  const addTierSelect = document.getElementById("ticket-detail-add-tier");
  if (
    ticket &&
    inferEventTicketType(ticket.event) === "Paid" &&
    addTierSelect
  ) {
    const tier =
      groupTicketTierOptions(ticket)[Number(addTierSelect.value || 0)] ||
      selectedDetailTier(ticket);
    payload = {
      ...payload,
      tierName: tier.name,
      ticketPrice: Number(tier.price || 0) || 0,
      serviceFee: Number(tier.fee || 0) || 0,
    };
  }
  const response = await postJson(`/api/tickets/${ticketId}/group`, payload);
  const nextTicket = upsertStoredTicket(response.ticket);
  await loadTickets();
  if (nextTicket) openTicketDetailsModal(nextTicket.id);
  return nextTicket;
}
async function payForSelectedGroup(ticketId) {
  const ticket = (state.tickets || []).find(
    (entry) => String(entry.id) === String(ticketId),
  );
  ...
  if (!selected.length) {
    setLocationStatus("Select at least one pending person first.", true);
    return;
  }
  const payload = { payForTicketIds: selected };
  if (ticket && inferEventTicketType(ticket.event) === "Paid") {
    const tier = selectedDetailTier(ticket);
    payload.tierName = tier.name;
    payload.ticketPrice = Number(tier.price || 0) || 0;
    payload.serviceFee = Number(tier.fee || 0) || 0;
  }
  try {
    const response = await postJson(`/api/tickets/${ticketId}/pay`, payload);
    const nextTicket = upsertStoredTicket(response.ticket);
    await loadTickets();
    if (nextTicket) openTicketDetailsModal(nextTicket.id);
  } catch (error) {
    setLocationStatus(error.message || "Failed to update this ticket.", true);
  }
}
function handleTicketCardClick(ticketId) {
  const ticket = (state.tickets || []).find(
  ...
  if (!ticket) return;
  openTicketDetailsModal(ticket.id);
}
const __originalSwitchMyEventsTab = switchMyEventsTab;
switchMyEventsTab = function (tabId) {
  __originalSwitchMyEventsTab(tabId);
  if (state.myEventsTab === "tickets") loadTickets();
};
window.__forceConfirmBooking = function (event) {
  if (event) {
    event.preventDefault();
    event.stopPropagation();
  }
...
if (!window.__finalGroupTicketBindingsBound) {
  ...
  document.addEventListener(
    ...
    (event) => {
      const bookingButton = event.target.closest(
        '#booking-action-btn,[data-action="confirm-booking"]',
      );
      if (bookingButton) {
        const bookingModal = document.getElementById("booking-modal");
        if (bookingModal && !bookingModal.classList.contains("hidden")) {
          ...
          if (!bookingButton.disabled) confirmBooking();
          return;
        }
      }
      const payButton = event.target.closest('[data-action="pay-ticket"]');
      if (payButton?.dataset.ticketId) {
        event.preventDefault();
        event.stopImmediatePropagation();
        openTicketDetailsModal(payButton.dataset.ticketId);
        return;
      }
      const addInviteeButton = event.target.closest(
        '[data-action="add-booking-invitee"]',
      );
      if (addInviteeButton?.dataset.userId) {
        event.preventDefault();
        event.stopImmediatePropagation();
        const userId = Number(addInviteeButton.dataset.userId || 0);
        const match = (bookingDraftState.searchResults || []).find(
          (user) => Number(user.userId) === userId,
        );
        if (
          match &&
          !(bookingDraftState.invitees || []).some(
            (user) => Number(user.userId) === userId,
          )
        ) {
      ...
      const removeInviteeButton = event.target.closest(
        '[data-action="remove-booking-invitee"]',
      );
      if (removeInviteeButton?.dataset.userId) {
        event.preventDefault();
        event.stopImmediatePropagation();
        const userId = Number(removeInviteeButton.dataset.userId || 0);
        bookingDraftState.invitees = (bookingDraftState.invitees || []).filter(
          (user) => Number(user.userId) !== userId,
        );
      ...
      const addGroupInviteeButton = event.target.closest(
        '[data-action="add-group-invitee"]',
      );
      if (
        addGroupInviteeButton?.dataset.userId &&
        addGroupInviteeButton?.dataset.ticketId
      ) {
        event.preventDefault();
        event.stopImmediatePropagation();
        const ticket = (state.tickets || []).find(
          (entry) =>
            String(entry.id) === String(addGroupInviteeButton.dataset.ticketId),
        );
        const inviteStatus =
          inferEventTicketType(ticket?.event) === "Free"
            ? document.getElementById("ticket-detail-add-status")?.value ||
              "confirmed"
            : "confirmed";
        updateGroupTicket(addGroupInviteeButton.dataset.ticketId, {
          inviteeUserIds: [Number(addGroupInviteeButton.dataset.userId || 0)],
          inviteeStatuses: {
            [Number(addGroupInviteeButton.dataset.userId || 0)]: inviteStatus,
          },
        }).catch((error) =>
          setLocationStatus(
            error.message || "Failed to add this person.",
            true,
          ),
        );
        return;
      }
      const removeGroupMemberButton = event.target.closest(
        '[data-action="remove-group-member"]',
      );
      if (
        removeGroupMemberButton?.dataset.userId &&
        removeGroupMemberButton?.dataset.ticketId
      ) {
        event.preventDefault();
        event.stopImmediatePropagation();
        updateGroupTicket(removeGroupMemberButton.dataset.ticketId, {
          removeUserIds: [Number(removeGroupMemberButton.dataset.userId || 0)],
        }).catch((error) =>
          setLocationStatus(
            error.message || "Failed to remove this person.",
            true,
          ),
        );
        return;
      }
      const paySelectedButton = event.target.closest(
        '[data-action="pay-selected-group"]',
      );
      if (paySelectedButton?.dataset.ticketId) {
        event.preventDefault();
        event.stopImmediatePropagation();
        payForSelectedGroup(paySelectedButton.dataset.ticketId);
        return;
      }
      const cancelButton = event.target.closest(
        '[data-action="cancel-ticket"]',
      );
      if (cancelButton?.dataset.ticketId) {
        event.preventDefault();
        event.stopImmediatePropagation();
        cancelTicket(cancelButton.dataset.ticketId);
        return;
      }
      const closeDetails = event.target.closest(
        '[data-action="close-ticket-details"]',
      );
      if (closeDetails) {
        event.preventDefault();
        closeTicketDetailsModal();
  ...
  document.addEventListener(
    ...
    (event) => {
      ...
      if (inviteStatusSelect?.dataset.userId) {
        bookingDraftState.inviteeStatuses[
          String(inviteStatusSelect.dataset.userId)
        ] =
          String(inviteStatusSelect.value || "confirmed")
            .trim()
            .toLowerCase() === "tentative"
            ? "tentative"
            : "confirmed";
      ...
      const pendingToggle = event.target.closest(
        '[data-action="toggle-pending-ticket"]',
      );
      if (pendingToggle?.dataset.participantTicketId) {
        const ticketId = String(pendingToggle.dataset.ticketId || "");
        const participantTicketId = String(
          pendingToggle.dataset.participantTicketId || "",
        );
        const next = new Set(
          groupTicketDetailState.selectedPendingTicketIds || [],
        );
        if (pendingToggle.checked) next.add(participantTicketId);
        else next.delete(participantTicketId);
        groupTicketDetailState.selectedPendingTicketIds = Array.from(next);
        openTicketDetailsModal(ticketId);
        return;
      }
      const tierSelect = event.target.closest("#ticket-detail-tier-select");
      if (tierSelect) {
        groupTicketDetailState.selectedTierIndex =
          Number(tierSelect.value || 0) || 0;
        if (groupTicketDetailState.ticketId)
          openTicketDetailsModal(groupTicketDetailState.ticketId);
      }
    },
    true,
  );
